 ...
 * @param map  百度地图
 * @param points 百度地图点集
 * @param markers 百度地图标注集，用来返回标注数组
 * @param contents 标注内容数组
 * @param color 颜色
 */
function addMarksMap(map,points,markers,contents){
    for(var i=0;i<points.length;i++){
        var point=points[i];
        var marker = new BMap.Marker(point);  // 创建标注
        map.addOverlay(marker);               // 将标注添加到地图中
        addClickHandler(map,contents[i],marker);	//为标注添加监听事件
        markers.push(marker);
    }
}
